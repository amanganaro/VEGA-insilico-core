...
//                        if (bnz_ring_count >= 4)
    ...
    public KmFactor() {
        ...
        // Init needed SMARTS
        // TODO: 25/06/2020 SMARTSParser.parse, object builder?
        Queries = new QueryAtomContainer[KmFragments.length];
    ...
    public void Calculate(InsilicoMolecule mol, DescriptorBlock FunctGroups) {
        ...
        try {
            ...
            for (int idx=0; idx<KmFragments.length; idx++) {
                ...
                // Fragment 1
                if ( (idx+1) == 1 )
                    if (Matcher.matches(Queries[idx])) curVal = Matcher.getUniqueMatchingAtoms().size();
                    else curVal = 0;
                ...
                // Fragment 21
                if ( (idx+1) == 21 ) 
                    if (Matcher.matches(Queries[idx])) {
                        if ( (Matcher.getUniqueMatchingAtoms().size() == mol.GetStructure().getAtomCount())) {
                            Matcher.matches(Query_benzene);
                            if (Matcher.getUniqueMatchingAtoms().size() >= 4)
                                curVal = 1;
                            else
                                curVal = 0;
                        } else
                ...
                if ( (idx+1) == 31 ) {
                    ...
                    for (int k=0; k<Queries_f_31.length; k++)
                        if (Matcher.matches(Queries_f_31[k])) curVal += Matcher.getUniqueMatchingAtoms().size();
                ...
                // Fragment 47
                if ( (idx+1) == 47 )
                    if (Matcher.matches(Queries[idx])) curVal = Matcher.getUniqueMatchingAtoms().size() * 3; // error in episuite?
                    else curVal = 0;
                ...
                if ( (idx+1) == 57 )
                    if (Matcher.matches(Queries[idx])) {
                        int nBiphenyls = Matcher.getUniqueMatchingAtoms().size();
                        curVal = nBiphenyls;
