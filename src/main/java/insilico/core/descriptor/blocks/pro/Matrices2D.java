                ...
                try {
                    ...
                    if (MatSymbol.equalsIgnoreCase("B")) {
                        ...
                        if (curMat[2].equalsIgnoreCase("v"))
                            w = (new insilico.core.descriptor.blocks.weights.basic.WeightsVanDerWaals()).getScaledWeights(curMol);
                        if (curMat[2].equalsIgnoreCase("e"))
                            w = (new insilico.core.descriptor.blocks.weights.basic.WeightsElectronegativity()).getScaledWeights(curMol);
                        if (curMat[2].equalsIgnoreCase("p"))
                            w = (new insilico.core.descriptor.blocks.weights.basic.WeightsPolarizability()).getScaledWeights(curMol);
                        if (curMat[2].equalsIgnoreCase("i"))
                            w = (new insilico.core.descriptor.blocks.weights.basic.WeightsIonizationPotential()).getScaledWeights(curMol);
                        // MANCA s
                        for (int i=0; i<nSK; i++)
                            Mat[i][i] = w[i];
                    }
