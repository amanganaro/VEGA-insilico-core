 ...
 * functionality. <p/>
 *
 * Original code taken from CDK and modified
 ...
 * Initialized with the Molecule that has to be checked, when checking
 * it has to be called passing a QueryAtomContainer, obtained from a SMARTS like:
 *
 * QueryAtomContainer query = SMARTSParser.parse(smarts);<p/><p/>
 *
 *
 * User code that wants to do SMARTS matching should use
 ...
 */
public class CustomQueryMatcher {
    ...
    public CustomQueryMatcher(InsilicoMolecule Mol) throws CDKException {
        this.curMol = Mol;
        try {
            initializeMolecule();
        } catch (Exception error) {
            throw new CDKException("Error initializing input molecule", error);
        }
    }
...
//    @TestMethod("testQueryTool, testQueryToolSingleAtomCase, testQuery")
    public boolean matches(QueryAtomContainer CurQuery) throws CDKException {
        ...
        try {
            ...
            initializeRecursiveSmarts(this.curMol.GetStructure());
     ...
     * <p/> This function should be called after IAtomContainer. If not,
...
//    @TestMethod("testQueryTool")
    ...
    public int countUniqueMatches() {
        List mappings = getUniqueMatchingAtoms();
...
//    @TestMethod("testUniqueQueries")
     ...
     *                      finding code.
     */
    private void initializeMolecule() throws Exception {
        // Code copied from
...
//        IRingSet sssr = finder.findEssentialRings();
        ...
        Cycles cycles = Cycles.sssr(curMol.GetStructure());
        RingSet sssr =  (RingSet) cycles.toRingSet();

        for (IAtom atom : curMol.GetStructure().atoms()) {
            ...
            List<IAtom> connectedAtoms = curMol.GetStructure().getConnectedAtomsList(atom);
        ...
        for (IBond bond : curMol.GetStructure().bonds()) {
        ...
        // this is not needed - passed molecules have already been checked
        // for aromaticity
