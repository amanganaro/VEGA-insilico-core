package insilico.core.similarity.descriptors;
 ...
 */
public class simConstitutional extends DescriptorBlock {

    private static final long serialVersionUID = 1L;
    private static final String BlockName = "Constitutional Descriptors (for Similarity Calculation)";
     ...
     */
    public simConstitutional() {
        super();
        this.Name = simConstitutional.BlockName;
    }
    ...
    public void Calculate(InsilicoMolecule mol) {
        ...
        try {
            ...
            // Weights sums
            double[] wMass = simWeightMass.getWeights(curMol);
            double HMass = simWeightMass.GetMass("H");
            double[] wVdW = simWeightVanDerWaals.getWeights(curMol);
            double HVdW = simWeightVanDerWaals.GetVdWVolume("H");
            double[] wPol = simWeightPolarizability.getWeights(curMol);
            double HPol = simWeightPolarizability.GetPolarizability("H");
            double[] wEl = simWeightElectronegativity.getWeights(curMol);
            double HEl = simWeightElectronegativity.GetElectronegativity("H");
...
//                wMassNS[i] = wMass[i] * CarbonWeight;
    ...
    public DescriptorBlock CreateClone()
            throws CloneNotSupportedException {
        simConstitutional block = new simConstitutional();
